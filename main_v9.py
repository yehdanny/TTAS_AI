"""
main_v9.py
Step 2 (v9): 標準主訴名稱精確比對優先，fallback RAG + Reranking。

v9 核心邏輯：
1. 啟動時從 ChromaDB 預載所有「標準主訴名稱 → chunk」的對應表
2. 對每筆病患「病人主訴」做 substring 精確比對：
   - 有比對到 → 直接使用匹配的 chunks（跳過 RAG）
   - 無比對到 → fallback：RAG TOP-5 + LLM Reranking（同 v8）
3. 後續流程不變：Criterion Selection → Rule-based Grade → Stage 2 if ★

執行方式：
    python main_v9.py           # 350 筆分層抽樣
    python main_v9.py --full    # 全量推理
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v9")
...
# 新格式：「大分類-標準主訴名稱」（如「心臟血管系統-心悸/不規則心跳」）
# group(1)=大分類, group(2)=標準主訴名稱
...
def build_complaint_name_index(collection) -> dict[str, list[dict]]:
    """
    從 ChromaDB 預載所有主訴 chunks，建立：
        { 標準主訴名稱: [ {doc, source}, ... ] }

    只載入主訴分級表（非總表）的 chunks，
    並過濾長度 < MIN_NAME_LEN 的名稱避免誤匹配。
    """
    COMPLAINT_SOURCES = set(
        ["外傷", "成人非外傷", "兒童", "環境"]
    )

    # ChromaDB 一次取出所有 documents（含 metadata）
    ...
    for doc, meta in zip(docs, metas):
        ...
        name = m.group(2).strip()   # group(2) = 標準主訴名稱（去代碼後）
        if len(name) < MIN_NAME_LEN:
            continue

        if name not in index:
            index[name] = []
        index[name].append({"doc": doc, "source": source})

    logging.info(
        f"Complaint name index built: {len(index)} unique 標準主訴名稱"
    )
    return index
...
def match_complaint_names(
    ...
    """
    在 patient_complaint 中搜尋所有標準主訴名稱（substring 比對）。
    依年齡過濾 source 後，回傳 (matched_docs, matched_names)。

    - 多個名稱匹配時全部取回，合併後送 criterion selection
    - 依長度降序排列名稱搜尋（較長名稱優先，減少短名稱的過度匹配）
    """
    ...
    # 依名稱長度降序搜尋（長名稱先匹配，命中後標記已覆蓋的區段）
    sorted_names = sorted(name_index.keys(), key=len, reverse=True)

    matched_docs: list[str] = []
    matched_names: list[str] = []
    covered_spans: list[tuple[int, int]] = []  # 已匹配的字元區段，避免重複
    ...
    for name in sorted_names:
        ...
        while True:
            ...
            if not overlapped:
                # 取符合年齡的 docs
                age_filtered = [
                    entry["doc"] for entry in name_index[name]
                    if entry["source"] in valid_sources
                ]
                if age_filtered:
...
def rerank_chunks(complaint: str, docs: list[str], llm: Llama) -> "tuple[int, str]":
    candidates_str = format_chunk_headers(docs)
    resp = llm.create_chat_completion(
        messages=[
            {"role": "system", "content": SYSTEM_RERANK},
            {"role": "user", "content": USER_RERANK.format(
                complaint=complaint, candidates=candidates_str
            )},
        ],
        temperature=0.1,
        max_tokens=16,
...
def parse_criteria_from_docs(docs: list[str]) -> list[dict]:
    """合併多個 docs 的判定依據，重新編號。"""
    all_criteria = []
    for doc in docs:
        for c in parse_criteria(doc):
            c["id"] = len(all_criteria) + 1
...
def main():
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        # ── 路徑 A：標準主訴名稱精確比對 ─────────────────────────────────────
        matched_docs, matched_names = match_complaint_names(
            rag_query, name_index, is_ped
        )
        retrieval_method = ""
        rerank_info = ""
        display_docs_header = ""
        ...
        if matched_docs:
            ...
            # 合併所有匹配到的 chunks 的判定依據
            criteria = parse_criteria_from_docs(matched_docs)
            display_docs_header = "\n".join(
                d.split("\n")[0] for d in matched_docs
            )
        else:
            # ── 路徑 B：RAG TOP-5 + Reranking（fallback）─────────────────────
        ...
        # ── 每筆完整 log ──────────────────────────────────────────────────────
        exact_tag = f"[{retrieval_method}]"
        if retrieval_method == "EXACT":
            match_info = f"命中標準主訴: {', '.join(matched_names)}\n匹配 chunk: {display_docs_header}"
        else:
            match_info = rerank_info + f"\nReranked TOP-1: {display_docs_header}"

        logging.info(
            f"\n[{idx + 1}/{len(sample)}] GT={true_level} | Pred={final_level} {exact_tag}\n"
    ...
    logging.info(
        f"Exact match hit: {n_exact_hit}/{len(y_true)} "
        f"({n_exact_hit / len(y_true) * 100:.1f}%)"
    )
    logging.info(
