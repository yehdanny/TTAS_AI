"""
main_v12.py
Step 2 (v12): 依官方 TTAS 成人/兒童基準寫死 Rule-based 生命徵象閾值。

v12 核心改進（相對於 v11）：
[I] 閾值依照官方標準文件寫死
    成人標準（data/rag_knowledge/成人判斷標準.txt）：
      ...
      Temp: 1級>41or<32; 2級>38or32~35
      GCS:  1級3~8; 2級9~13
    兒童標準（data/rag_knowledge/小兒判斷標準.txt）：
      SBP:  1級<70 (>1歲)
      SpO2: 同成人
      RR:   1級<10
      HR:   依年齡分段（<3m / 3m~3y / >3y）
      Temp: 1級>41or<32; 2級依月齡分（<3m: >38 or 32~36; >=3m: 32~35）
      GCS:  同成人
[J] 雙軌制（繼承 v11）：Vital 軌 + LLM 軌 → final=min(vital_min, llm_level)
[K] VitalAlert 顯示等級數字取代「異常」文字
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v12")
...
def assess_vitals(
    ...
    if "temp" in nv:
        ...
        if ped:
            ...
            elif t > 38 or (32 <= t <= 35):
                alerts.append(VitalAlert("Temp", t, "異常", 2,
                    f"體溫={t:.1f}°C >38 or 32~35 → 2級"))
...
def main():
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        alerts = assess_vitals(nv, is_ped, age_years=age_years)
