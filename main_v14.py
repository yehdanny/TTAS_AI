"""
main_v14.py
Step 2 (v14): 修正 Temp>38 假陽性問題、新增 MOI 關鍵字偵測、改善 Lv2 提示詞、移植疼痛分數支援。

v14 核心改進（相對於 v12）：
[L] 成人 Temp 規則修正：移除 >38 -> 2級（需 SIRS/免疫缺陷才成立，純數值無法判定）
    保留：>41/<32 -> 1級；32~35 -> 2級（低體溫客觀指標）
[M] MOI 關鍵字偵測（第三條 Lv2 規則軌道）：
    槍傷/被彈出車外/頭頸軀幹穿刺傷/危及生命大量出血 -> moi_level=2
    final = min(vital_min, moi_level, llm_level)
[N] SYSTEM_SELECTION prompt 強化：
    明確允許 MOI/大量出血案例即使生命徵象暫時穩定也可選 Lv2
    增加 Lv5 選擇指引（明確輕症+正常生命徵象時優先考慮4/5級）
[O] 疼痛分數支援（移植 v13）：
    若 CSV/API 有疼痛欄位則啟動 rule-based 疼痛分級；否則停用

繼承 v12：A-K 全部保留（SBP/MAP/DBP/SpO2/HR/RR/GCS 官方閾值、雙軌制、EXACT 命中等）

執行方式：
    python main_v14.py
    python main_v14.py --full
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v14")
...
def main():
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        logging.info(
            f"\n{'='*50}\n"
            f"[{idx+1}/{len(sample)}] GT={true_level} | Vital={vital_min}"
            f"{f' MOI={moi_level}' if moi_level==2 else ''}"
            f" | v_min={v_min} | LLM={llm_level} | "
            f"Final={final_level}  [{retrieval_method}] {correct_mark}\n"
            f"{'='*50}\n"
            f"[Patient]\n"
            f"{vitals_text}\n"
