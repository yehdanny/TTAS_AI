"""
main_v11.py
Step 2 (v11): 全面 Rule-based 生命徵象評估 + 精準 Criteria 過濾。

v11 核心改進：
[E] 全面 Vitals Assessment：SpO2、GCS、MAP/SBP/DBP、HR、RR、Temp
    每個指標對照正常範圍，輸出正常/異常標注 + 建議最低緊急度 (vital_min_level)
[F] Vitals Text 標注：每個數值附上「正常/偏低/偏高/異常」標籤，
    讓 LLM 直接知道生命徵象狀態，不需自行推斷
[G] 擴充 Pre-filter：
    - HR 正常 → 刪除「血壓或心跳有異於平常數值」catch-all
    - Temp 正常 → 刪除含「高燒/發燒」的 criteria
    - HR/BP/SpO2 任一異常 → 刪除 Lv5「生命徵象正常」criteria
    - Temp ≥ 37.5 → 刪除 Lv5「無發燒」criteria
[H] Vital 強制約束：若 vital_min_level ≤ 2，限制 LLM 只從 ≤2 級 criteria 中選擇
    （避免 SpO2=69% 的病人被預測成 Lv4/5）

繼承 v10：B（pre-filter）、C（prompt 有據才選）、D（EXACT 多命中 reranking）

執行方式：
    python main_v11.py
    python main_v11.py --full
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v11")
...
def assess_vitals(nv: dict, is_pediatric: "Optional[bool]" = False) -> "list[VitalAlert]":
    """
    [E] 對每個可用生命徵象與正常範圍比對，產生 VitalAlert 清單。

    成人最低建議 TTAS 等級規則：
      Lv2 觸發：SpO2<90, GCS≤8, MAP<65, SBP<90, HR>150 or <40, RR>36 or <8, Temp>40.5 or <35
      Lv3 觸發：SpO2 90-94, GCS 9-13, MAP 65-70, SBP 90-100, HR 121-150 or 41-50,
                RR 25-36, Temp 38.5-40.5 or 35-36

    小兒（is_pediatric=True）閾值依 TTAS 兒童分類基準調整：
      心跳（HR）正常上限 140，血壓偏低、呼吸速率偏快均為正常
      Lv2 觸發：HR>160 or <50, SBP<70 or MAP<50, RR>40 or <10
      Lv3 觸發：HR>130 or <55, SBP<80 or MAP<60, RR>30 or <12
    """
    alerts = []
    ped = bool(is_pediatric)

    # SpO2（成人兒童相同）
    # < 85% 危及生命 → Lv1；85-89% 重度 → Lv2；90-94% 中輕度 → Lv2/3
    if "sao2" in nv:
        s = nv["sao2"]
        if s < 85:
            alerts.append(VitalAlert("SpO2", s, "異常", 1, f"SpO2={s:.1f}% < 85% → 危及生命 → 至少1級"))
        elif s < 90:
            alerts.append(VitalAlert("SpO2", s, "異常", 2, f"SpO2={s:.1f}% < 90% → 重度呼吸窘迫 → 至少2級"))
        elif s < 92:
            alerts.append(VitalAlert("SpO2", s, "異常", 2, f"SpO2={s:.1f}% < 92% → 中度呼吸窘迫 → 至少2級"))
        elif s < 95:
            alerts.append(VitalAlert("SpO2", s, "偏低", 3, f"SpO2={s:.1f}% 92-94% → 輕度呼吸窘迫 → 至少3級"))

    # GCS（成人兒童相同）
    # ≤ 8 需要急救（Lv1）；9-13 意識改變（Lv2）
    if "gcs" in nv:
        g = nv["gcs"]
        if g <= 8:
            alerts.append(VitalAlert("GCS", g, "異常", 1, f"GCS={g} ≤ 8 → 意識嚴重障礙，需急救 → 至少1級"))
        elif g <= 13:
            alerts.append(VitalAlert("GCS", g, "異常", 2, f"GCS={g} 9-13 → 意識改變 → 至少2級"))

    # MAP / SBP（兒童閾值偏低）
    # MAP < 65 或 SBP < 90 表示血行動力不穩定（Lv1）
    if "map" in nv:
        m = nv["map"]
        sbp = nv.get("sbp", 999)
        if ped:
            if m < 50 or sbp < 70:
                alerts.append(VitalAlert("MAP", m, "異常", 1,
                    f"MAP={m:.1f} < 50 或 SBP={sbp:.0f} < 70 → 兒童血行動力不穩定 → 至少1級"))
            elif m < 60 or sbp < 80:
                alerts.append(VitalAlert("MAP", m, "偏低", 2,
                    f"MAP={m:.1f} 50-60 / SBP={sbp:.0f} 70-80 → 兒童血壓偏低 → 至少2級"))
        else:
            if m < 65 or sbp < 90:
                alerts.append(VitalAlert("MAP", m, "異常", 1,
                    f"MAP={m:.1f} < 65 或 SBP={sbp:.0f} < 90 → 血行動力不穩定 → 至少1級"))
            elif m < 70 or sbp < 100:
                alerts.append(VitalAlert("MAP", m, "偏低", 2,
                    f"MAP={m:.1f} 65-70 / SBP={sbp:.0f} 90-100 → 血壓偏低 → 至少2級"))
    ...
    if "hr" in nv:
        ...
        if ped:
            if h > 160 or h < 50:
                alerts.append(VitalAlert("HR", h, "異常", 1, f"HR={h:.0f} 兒童極端異常 → 至少1級"))
            elif h > 130 or h < 55:
                alerts.append(VitalAlert("HR", h, "偏高" if h > 130 else "偏低", 2,
                    f"HR={h:.0f} 兒童明顯異常 → 至少2級"))
        else:
            if h > 160 or h < 35:
                alerts.append(VitalAlert("HR", h, "異常", 1, f"HR={h:.0f} 極端異常 → 至少1級"))
            elif h > 150 or h < 40:
                alerts.append(VitalAlert("HR", h, "異常", 2, f"HR={h:.0f} 嚴重異常 → 至少2級"))
            elif h > 120 or h < 50:
                alerts.append(VitalAlert("HR", h, "偏高" if h > 120 else "偏低", 3,
                    f"HR={h:.0f} 明顯異常 → 至少3級"))
    ...
    if "rr" in nv:
        ...
        if ped:
            if r > 40 or r < 10:
                alerts.append(VitalAlert("RR", r, "異常", 1, f"RR={r:.0f} 兒童極端異常 → 至少1級"))
            elif r > 30 or r < 12:
                alerts.append(VitalAlert("RR", r, "偏高" if r > 30 else "偏低", 2,
                    f"RR={r:.0f} 兒童明顯異常 → 至少2級"))
        else:
            if r > 36 or r < 6:
                alerts.append(VitalAlert("RR", r, "異常", 1, f"RR={r:.0f} 極端異常 → 至少1級"))
            elif r > 30 or r < 8:
                alerts.append(VitalAlert("RR", r, "異常", 2, f"RR={r:.0f} 嚴重異常 → 至少2級"))
            elif r > 24 or r < 10:
                alerts.append(VitalAlert("RR", r, "偏高" if r > 24 else "偏低", 3,
                    f"RR={r:.0f} 明顯異常 → 至少3級"))
    ...
    if "temp" in nv:
        t = nv["temp"]
        if t > 41.0 or t < 34.0:
            alerts.append(VitalAlert("Temp", t, "異常", 1, f"體溫={t:.1f}°C 極端異常 → 至少1級"))
        elif t > 40.5 or t < 35.0:
            alerts.append(VitalAlert("Temp", t, "異常", 2, f"體溫={t:.1f}°C 嚴重異常 → 至少2級"))
        elif t >= 38.5 or t < 36.0:
            alerts.append(VitalAlert("Temp", t, "異常", 3, f"體溫={t:.1f}°C 異常 → 至少3級"))
        elif t >= 37.5:
            alerts.append(VitalAlert("Temp", t, "偏高", 4, f"體溫={t:.1f}°C 低燒 → 至少4級"))
...
_VITAL_FILTER_RULES: list[tuple] = [
    ...
    # Temp
    (re.compile(r"高燒|[>＞]\s*38\.5|體溫.*38"), "temp", lambda v: v > 38.5, "Temp>38.5"),
    (re.compile(r"低體溫|[<＜]\s*36"),            "temp", lambda v: v < 36,  "Temp<36"),
]
...
def _make_extended_vitals(nv: dict, is_pediatric: "Optional[bool]" = False) -> dict:
    ...
    兒童使用 HR 60-140、SBP 80-120 的正常範圍。
    """
    ev = dict(nv)
    if is_pediatric:
        hr_normal = 60 <= nv.get("hr", 100) <= 140 if "hr" in nv else None
        sbp_normal = 80 <= nv.get("sbp", 100) <= 120 if "sbp" in nv else None
        map_normal = nv.get("map", 70) >= 55 if "map" in nv else None
    else:
        hr_normal = 60 <= nv.get("hr", 80) <= 100 if "hr" in nv else None
        sbp_normal = 90 <= nv.get("sbp", 120) <= 140 if "sbp" in nv else None
...
def main():
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        alerts = assess_vitals(nv, is_ped)
        ...
        correct_mark = "✓" if final_level == true_level else "✗"
