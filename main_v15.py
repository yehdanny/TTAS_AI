"""
main_v15.py
Step 2 (v15): 三分類實驗版（Lv1 / Lv2 / Lv3+）。

由於 Lv3/Lv4/Lv5 高度依賴疼痛分數（占比 42-95%），而現有資料集無此欄位，
v15 將三者合併為「Lv3+（非緊急/次緊急/緊急合計）」進行三分類評估，
以釐清模型在可量化維度（生命徵象 + 主訴語意）上的真實上限。

v15 核心改動（相對於 v14）：
[P] 抽樣改為三分類平衡：Lv1×70 + Lv2×70 + Lv3-5合併×70 = 210 筆
[Q] 預測 pipeline 不變（仍輸出 1-5），final 後 collapse：3/4/5 → 3
[R] 評估指標改為 3-class（labels=[1,2,3]）
[S] 混淆矩陣與報告標籤對應：1=復甦急救, 2=危急, 3=緊急/次緊急/非緊急合計

繼承 v14：L/M/N/O 全部保留

執行方式：
    python main_v15.py
    python main_v15.py --full
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v15")

# ── 參數 ───────────────────────────────────────────────────────────────────────
EMBED_MODEL = "BAAI/bge-m3"
COLLECTION_NAME = "ttas_v7"
INSTRUCTION_PREFIX = "为这个句子生成表示以用于检索相关文章："
SAMPLE_PER_LEVEL = 70   # 每個分類各取 70 筆（三分類：Lv1/Lv2/Lv3+）
RANDOM_STATE = 42

# [P] 三分類設定
COLLAPSE_MAP = {1: 1, 2: 2, 3: 3, 4: 3, 5: 3}  # 3/4/5 → 3
CLASS_LABELS = [1, 2, 3]
CLASS_NAMES  = {1: "Lv1(復甦急救)", 2: "Lv2(危急)", 3: "Lv3+(緊急/次緊急/非緊急)"}
...
# [O] 疼痛分數欄位候選（依序比對 CSV/API 欄位名稱；若無則停用 rule-based 疼痛判定）
PAIN_SCORE_COL_CANDIDATES = [
    "疼痛指數", "疼痛分數", "NRS疼痛分數", "NRS", "VAS", "pain_score", "疼痛評分"
]
...
3. 若判定依據涉及【高危受傷機轉】（如：高處墜落、被彈出車外、槍傷、
   頭頸軀幹穿刺傷）或【危及生命的大量出血】，即使目前生命徵象暫時穩定，
   仍應選擇2級條目（外傷後可能快速惡化）。
4. 若整體生命徵象穩定且主訴無前述高危情況，不應選擇需要血行動力不穩定、
   意識改變、重度呼吸窘迫等嚴重條件的條目。
5. 若無法確定某嚴重條件是否成立，以穩定的生命徵象為準，選擇較保守的條目。
6. 若病患生命徵象全部正常且主訴明確輕微，請優先考慮4或5級條目。
7. 只回答一個整數（條目編號），不要輸出任何其他文字。"""
...
def assess_vitals(
    ...
    if ped:
        ...
        if sbp is not None:
            if sbp < 70:
                alerts.append(VitalAlert("SBP", sbp, "異常", 1, f"SBP={sbp:.0f} < 70 → 1級"))
            elif sbp < 90:
                alerts.append(VitalAlert("SBP", sbp, "偏低", 2, f"SBP={sbp:.0f} < 90 → 2級"))
            elif sbp >= 220:
                alerts.append(VitalAlert("SBP", sbp, "異常", 3, f"SBP={sbp:.0f} ≥ 220 → 3級（高血壓危象）"))
            elif sbp >= 200:
                alerts.append(VitalAlert("SBP", sbp, "偏高", 4, f"SBP={sbp:.0f} 200~220 → 4級"))
        # 成人 MAP
        if "map" in nv:
            m = nv["map"]
            if m < 65:
        ...
        if "dbp" in nv:
            d = nv["dbp"]
            if d >= 130:
                alerts.append(VitalAlert("DBP", d, "異常", 3, f"DBP={d:.0f} ≥ 130 → 3級（高血壓危象）"))
            elif d >= 110:
                alerts.append(VitalAlert("DBP", d, "偏高", 4, f"DBP={d:.0f} 110~130 → 4級"))
    ...
    if "hr" in nv:
        ...
        if ped:
            ...
            # 成人：1級需同時 SBP<70（複合條件）
            hr_abnormal = (h < 50 or h > 140)
            if hr_abnormal and sbp is not None and sbp < 70:
                alerts.append(VitalAlert("HR", h, "異常", 1,
                    f"HR={h:.0f} (HR<50 or >140) AND SBP={sbp:.0f}<70 → 1級"))
            elif hr_abnormal:
                alerts.append(VitalAlert("HR", h, "異常", 2,
                    f"HR={h:.0f} <50 or >140 → 2級"))
        ...
        if ped:
            ...
            if t > 41 or t < 32:
                alerts.append(VitalAlert("Temp", t, "異常", 1,
                    f"體溫={t:.1f}°C >41 or <32 → 兒童1級"))
            elif age_m is not None and age_m < 3:
                # <3個月：>38 or 32~36 → 2級
                if t > 38 or (32 <= t <= 36):
                    alerts.append(VitalAlert("Temp", t, "異常", 2,
                        f"體溫={t:.1f}°C 嬰兒(<3月)2級 (>38 or 32~36)"))
            else:
                # ≥3個月：32~35 → 2級（>38 不自動觸發 Lv2）
                if 32 <= t <= 35:
                    alerts.append(VitalAlert("Temp", t, "偏低", 2,
                        f"體溫={t:.1f}°C 兒童(≥3月)2級 (32~35°C)"))
        else:
            # 成人
            # 注意：TTAS 成人 Temp>38 需同時符合「免疫功能缺陷」或「SIRS≥3項」才觸發 2級
            # 純數值無法評估這兩個臨床條件，故移除 >38 的自動 Lv2 觸發
            # 僅保留低體溫（32~35°C）可純數值判定的 Lv2 條件
            if t > 41 or t < 32:
                alerts.append(VitalAlert("Temp", t, "異常", 1,
                    f"體溫={t:.1f}°C >41 or <32 → 1級"))
            elif 32 <= t <= 35:
                alerts.append(VitalAlert("Temp", t, "偏低", 2,
                    f"體溫={t:.1f}°C 32~35 → 2級（低體溫）"))

    # ── [O] 疼痛分數（僅在 CSV/API 有提供時啟動）─────────────────────────────
    if pain_score is not None:
        if ped:
            # 兒童：不區分中樞/周邊型
            if pain_score >= 8:
                alerts.append(VitalAlert("疼痛", pain_score, "重度", 2,
                    f"疼痛={pain_score:.0f} 重度(>=8) → 兒童2級"))
            elif pain_score >= 4:
                alerts.append(VitalAlert("疼痛", pain_score, "中度", 3,
                    f"疼痛={pain_score:.0f} 中度(4~7) → 兒童3級"))
            elif pain_score > 0:
                alerts.append(VitalAlert("疼痛", pain_score, "輕度", 4,
                    f"疼痛={pain_score:.0f} 輕度(<4) → 兒童4級"))
        else:
            # 成人：保守採「中樞型」標準（NRS 僅代表強度，類型需臨床判斷）
            if pain_score >= 8:
                alerts.append(VitalAlert("疼痛", pain_score, "重度", 2,
                    f"疼痛={pain_score:.0f} 重度中樞型(>=8) → 成人2級"))
            elif pain_score >= 4:
                alerts.append(VitalAlert("疼痛", pain_score, "中度", 3,
                    f"疼痛={pain_score:.0f} 中度中樞型(4~7) → 成人3級"))
            elif pain_score > 0:
                alerts.append(VitalAlert("疼痛", pain_score, "輕度", 4,
                    f"疼痛={pain_score:.0f} 輕度中樞型(<4) → 成人4級"))
...
# ── [M] MOI 關鍵字偵測 ────────────────────────────────────────────────────────
# 依 TTAS 總表：以下主訴關鍵字可純文字判定為「高危受傷機轉」→ 至少 2 級
# 僅納入高確定性關鍵字，避免誤觸（保持 precision）
_MOI_LV2_PATTERNS: list[re.Pattern] = [
    re.compile(r"槍傷"),
    re.compile(r"被彈出車外"),
    re.compile(r"(頭|頸|胸|腹|脊椎|軀幹).{0,8}(穿刺|刺傷)"),
    re.compile(r"(穿刺|刺傷).{0,8}(頭|頸|胸|腹|脊椎|軀幹)"),
    re.compile(r"大量出血"),
    re.compile(r"危及生命.{0,6}出血|出血.{0,6}危及生命"),
]


def detect_moi_level(complaint: str) -> int:
    """
    [M] 從主訴文字偵測高危受傷機轉（High-risk MOI）。
    若命中任一 Lv2 模式 → 回傳 2；否則回傳 5（不觸發）。
    """
    if not complaint or complaint == "不詳":
        return 5
    for pattern in _MOI_LV2_PATTERNS:
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    axes[0].set_title("Confusion Matrix (Counts) - 3-class")
    axes[0].set_xlabel("Predicted")
    axes[0].set_ylabel("True")
    sns.heatmap(cm_norm, annot=True, fmt=".2f", cmap="Oranges",
                xticklabels=tick_labels, yticklabels=tick_labels, ax=axes[1], vmin=0, vmax=1)
    axes[1].set_title("Confusion Matrix (Row-Normalized %) - 3-class")
...
def compute_metrics(y_true, y_pred):
    ...
    # 三分類：adjacent accuracy = |pred-true| <= 1（Lv1↔Lv2 相差1，Lv2↔Lv3+ 相差1）
...
def save_metrics_report(metrics, save_path):
    lines = [
        "="*60, "TTAS 檢傷分級 LLM 評估報告v15（三分類：Lv1/Lv2/Lv3+）", "="*60,
        ...
        "", "─"*40, "各類別指標（僅計有效預測）：",
        f"{'Class':<18}  {'Precision':>10}  {'Recall':>8}  {'F1':>8}  {'Support':>8}",
    ]
    for i, lv in enumerate(metrics["labels"]):
        name = CLASS_NAMES.get(lv, f"Lv{lv}")
        lines.append(
            f"  {name:<18}  {metrics['precision'][i]:>10.4f}  "
...
def main():
    ...
    if pain_col:
        logging.info(f"[O] Pain score column found: '{pain_col}' -> rule-based pain active")
    else:
        logging.info("[O] Pain score column not found -> pain rule-based inactive")

    if FULL_RUN:
        sample = df.reset_index(drop=True)
    else:
        # [P] 三分類平衡抽樣：Lv1×70 + Lv2×70 + Lv3-5合併×70 = 210 筆
        s1 = df[df[triage_col] == 1].sample(
            n=min(SAMPLE_PER_LEVEL, (df[triage_col] == 1).sum()), random_state=RANDOM_STATE)
        s2 = df[df[triage_col] == 2].sample(
            n=min(SAMPLE_PER_LEVEL, (df[triage_col] == 2).sum()), random_state=RANDOM_STATE)
        s3 = df[df[triage_col].isin([3, 4, 5])].sample(
            n=min(SAMPLE_PER_LEVEL, df[triage_col].isin([3, 4, 5]).sum()), random_state=RANDOM_STATE)
        sample = pd.concat([s1, s2, s3]).reset_index(drop=True)
    logging.info(f"\nSampled {len(sample)} records (3-class: Lv1={len(s1 if not FULL_RUN else df[df[triage_col]==1])}, "
                 f"Lv2={len(s2 if not FULL_RUN else df[df[triage_col]==2])}, "
                 f"Lv3+={len(s3 if not FULL_RUN else df[df[triage_col].isin([3,4,5])])}).")
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        if pain_col:
            try:
                ps_val = float(row.get(pain_col))
                if not np.isnan(ps_val) and ps_val >= 0:
                    pain_score = ps_val
            except (TypeError, ValueError):
                pass

        alerts = assess_vitals(nv, is_ped, age_years=age_years, pain_score=pain_score)
        vital_min = vital_min_level(alerts)

        # [M] MOI 關鍵字偵測（第三條 Lv2 軌道）
        moi_level = detect_moi_level(rag_query)
        v_min = min(vital_min, moi_level)
        ...
        # [Q] 三分類 collapse：3/4/5 → 3
        y_true.append(COLLAPSE_MAP.get(true_level, true_level))
        y_pred.append(COLLAPSE_MAP.get(final_level, final_level) if final_level != -1 else -1)
        ...
        records.append({
            ...
            "vital_track": vital_min, "moi_track": moi_level,
            "combined_vital_min": v_min, "llm_track": llm_level,
        ...
        SEP = "─" * 50
        alert_lines = (
            "  " + "、".join(a.label() for a in alerts) + f" → vital_min={vital_min}級"
            if alerts else "  （無異常警示）"
        )
        moi_info = f"  [M] MOI偵測: {moi_level}級" if moi_level == 2 else ""
        criteria_flow = (
        ...
        logging.info(
            f"\n{'='*50}\n"
            f"[{idx+1}/{len(sample)}] GT={true_level}->{COLLAPSE_MAP.get(true_level,true_level)} | Vital={vital_min}"
            f"{f' MOI={moi_level}' if moi_level==2 else ''}"
            f" | v_min={v_min} | LLM={llm_level} | "
            f"Final={final_level}->{COLLAPSE_MAP.get(final_level,final_level) if final_level!=-1 else -1}  [{retrieval_method}] {correct_mark}\n"
            f"{'='*50}\n"
            f"[Patient]\n"
            f"{vitals_text}\n"
            f"  [!] Vital alerts: {alert_lines}\n"
            f"{moi_info}\n"
            f"{SEP}\n"
            ...
            f"{SEP}\n"
            f"[Final] min(vital={vital_min}, moi={moi_level}, LLM={llm_level}) = {final_level}  {correct_mark}\n"
        )
