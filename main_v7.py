"""
main_v7.py
Step 2 (v7): 結構化兩階段推理 + logging。

v7 核心改進：
1. RAG query 只用「病人主訴」，embedding 用大分類+主訴名稱建立（build_rag_v7.py）
2. 推理拆成兩步：
   Step A - Criterion Selection：LLM 從 Top-1 chunk 的判定依據清單中選出最符合的編號
   Step B - Grade Extraction：rule-based 從選中的條目取出等級 + ★ flag
3. ★ 偵測 rule-based（從 parse_criteria 的 has_star 欄位）
4. Stage 2（次要調節變數）只在選中條目 has_star=True 時觸發
5. 改用 logging 儲存 log 至 results_v7/run.log

執行方式：
    python main_v7.py           # 350 筆分層抽樣
    python main_v7.py --full    # 全量推理
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_v7")
...
# Step A：LLM 判定所有可成立的依據，回傳其中等級最小（最緊急）的編號
SYSTEM_SELECTION = """\
...
# Step B（Stage 2）：當選中條目有 ★ 時，用次要調節變數規則決定最終等級
SYSTEM_SECONDARY = """\
...
def build_vitals_text(row: pd.Series) -> str:
    """生理數值字串，送給 LLM 用於與判定依據比對。只輸出有值欄位。"""
    parts = []
...
def retrieve_complaint_chunks(
    ...
    """
    Stage 1：只用病患主訴查詢，回傳 (docs_list, top1_doc)。
    embedding 比對 ChromaDB 中以「大分類+主訴名稱」建立的向量。
    """
...
def parse_criteria(chunk_text: str) -> list[dict]:
    """
    從 chunk 文字中 rule-based 解析判定依據清單。

    回傳 list of:
        {"id": int, "criteria": str, "level": int, "has_star": bool}
    """
    criteria = []
...
def format_criteria_list(criteria: list[dict]) -> str:
    """將結構化判定依據格式化成給 LLM 看的編號清單。"""
    lines = []
    for c in criteria:
        lines.append(f"{c['id']}. {c['criteria']}")
    return "\n".join(lines)


# ── LLM 回應解析 ──────────────────────────────────────────────────────────────
def parse_selection(text: str, n_criteria: int) -> int:
    """從 LLM 回應解析選中的判定依據編號（1-based）；無法解析回傳 -1。"""
...
def main():
    ...
    for idx, (_, row) in enumerate(sample.iterrows()):
        ...
        # ── Stage 1: 取主訴 chunks ────────────────────────────────────────────
        complaint_docs, top1_doc = retrieve_complaint_chunks(
            rag_query, embedder, collection, is_ped
        )

        # ── Rule-based: parse 判定依據清單 ────────────────────────────────────
        criteria = parse_criteria(top1_doc)

        # ── Step A: LLM 選擇最符合的判定依據編號 ─────────────────────────────
        raw_selection = ""
        ...
        if criteria:
            ...
            sel_idx = parse_selection(raw_selection, len(criteria))
            ...
            if sel_idx != -1:
                ...
                # 選擇失敗 → fallback: 使用 Top-1 chunk 中最低等級（最嚴重）的條目
                final_level = min(c["level"] for c in criteria) if criteria else -1
        else:
            # Top-1 chunk 無法解析 criteria → 直接用 LLM 判斷
            context_str = "\n---\n".join(complaint_docs)
