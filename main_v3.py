...
main.py
main_v3.py
Step 2 (v3): 使用手動 chunks RAG，分層抽樣 → RAG 檢索 → LLM 推理 → 評估輸出。
...
【TTAS 判定流程】
1. 根據病人主訴與生命徵象，在參考指引中找到最匹配的判定依據及其 TTAS 級數。
2. 若該判定依據標有「★需查次要調節變數規則」，則需進一步參考參考指引中的次要調節變數規則，決定最終級數是否調整。
3. 若判定依據無★標記，直接輸出查到的 TTAS 級數。
...
def build_query(row: pd.Series) -> str:
    """從病患資料列建立查詢字串。"""
    gcs_total = safe_val(row.get("GCS_E", np.nan))
    try:
    ...
    gender = safe_val(row.get("性別", np.nan))
